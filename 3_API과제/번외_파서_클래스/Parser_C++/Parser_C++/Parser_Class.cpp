...
// 파일 로드
void Parser::LoadFile(TCHAR* FileName)
{
	FILE* fp;			// 파일 스트림
...
// 구역 지정
bool Parser::AreaCheck(TCHAR* AreaName)
{
	int i = 0;
	TCHAR cTempWord[256];
	int iStartPos;
...
// 파서_원하는 값 찾아오기 (int형)
bool Parser::GetValue_Int(TCHAR* Name, int* value)
{
	m_ilen = 0;
...
// 파서_원하는 값 찾아오기 (문자열)
bool Parser::GetValue_String(TCHAR* Name, TCHAR* value)
{
	m_ilen = 0;
