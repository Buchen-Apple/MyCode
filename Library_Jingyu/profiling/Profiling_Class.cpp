...
#define MIN_SET 100000000
	...
	void PROFILING_FILE_SAVE()
	{
		...
		fprintf_s(wStream, "%30s  |%12s  |%11s   |%11s   |%11s |", "Name", "Average", "Min", "Max", "Call");
		fprintf_s(wStream, "\n---------------------------------------------------------------------------- ");
		...
		for (int i = 0; i <= g_NowProfiling_Size; ++i)
		{
			...
			fprintf_s(wStream, "\n%31s |%11.3lf㎲ |%11.3lf㎲ |%11.3lf㎲ |%11d |", profile[i].m_Name, profile[i].m_TotalTime / profile[i].m_CallCount, profile[i].m_MInTime[1], profile[i].m_MaxTime[1], profile[i].m_CallCount);
		}
