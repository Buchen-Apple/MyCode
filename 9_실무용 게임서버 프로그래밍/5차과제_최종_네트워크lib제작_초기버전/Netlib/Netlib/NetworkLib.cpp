...
#define	Struct_LockSession()	LockSession_Func()
#define Struct_UnlockSession()	UnlockSession_Func()
...
bool CLanServer::Start(const TCHAR* bindIP, USHORT port, int WorkerThreadCount, bool Nagle, int MaxConnect)
{
	// 윈속 초기화
	WSADATA wsa;
...
bool CLanServer::SendPacket(ULONGLONG ClinetID, CProtocolBuff* SendBuff)
{
	...
	int EnqueueCheck = NowSession->m_SendQueue.Enqueue(SendBuff->GetBufferPtr(), Size);
	if (EnqueueCheck == -1)
	{
