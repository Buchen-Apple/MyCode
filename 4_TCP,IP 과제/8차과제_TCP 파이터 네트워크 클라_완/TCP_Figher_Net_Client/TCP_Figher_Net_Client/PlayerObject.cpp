...
void PlayerObject::ActionProc()
{
	...
	if (m_bAttackState == TRUE)
	{
		...
		{
			// 내 캐릭터일 경우, 메시지를 강제로 이전 메시지로 변환.즉, 공격 중에는 다른 액션 모두 무시
			...
			{
				// 현재 메시지가 이동 메시지라면
				if (m_dwActionCur == dfACTION_MOVE_LL || m_dwActionCur == dfACTION_MOVE_LU || m_dwActionCur == dfACTION_MOVE_LD ||
					m_dwActionCur == dfACTION_MOVE_RR || m_dwActionCur == dfACTION_MOVE_RU || m_dwActionCur == dfACTION_MOVE_RD ||
					m_dwActionCur == dfACTION_MOVE_UU || m_dwActionCur == dfACTION_MOVE_DD)
				{
					//  공격 상태 해제
					m_bAttackState = FALSE;
					m_dwNowAtkType = 0;
		...
		{			
			...
			{
				// 현재 메시지가 아직 공격이라면, 현재 상태를 IDLE로 변경
				if (m_dwActionCur == dfACTION_ATTACK_01_LEFT || m_dwActionCur == dfACTION_ATTACK_02_LEFT || m_dwActionCur == dfACTION_ATTACK_03_LEFT)
					m_dwActionCur = dfACTION_IDLE;
			}

			// 내 캐릭터라면 현재 상태를 IDLE로 변경
			else
				m_dwActionCur = dfACTION_IDLE;

			// 내 캐릭터 / 상대 캐릭터 모두, 공격 모드 해제
			m_bAttackState = FALSE;
			m_dwNowAtkType = 0;
		}			
	}
	...
	case dfACTION_IDLE:
		...
		if (m_bAttackState == FALSE && m_bIdleState == FALSE)
		{
			...
			// 내 캐릭터이고, 이동 액션이 아이들이 아니라면 대기 시작 데이터 Send
