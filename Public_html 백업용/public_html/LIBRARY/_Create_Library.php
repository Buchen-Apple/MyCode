...
{   
    $QueryArray = array("INSERT INTO `$dbName`.`account`(`accountno`, `email`) VALUES ($AccountNo, '$email' )", 
                        "INSERT INTO `$dbName`.`contents`(`accountno`) VALUES ($AccountNo)");


    if(DB_TransactionQuery($QueryArray, $shDB_Data, $AccountNo) === false)
    {
        ...
        {
            // 시스템로그 남김
            $errorstring = 'DataSet--> TBL Value Duplicate.';
...
function MinusAvailable($dbno)
{
     ...
     global $Info_Master_DB_IP, $Info_Master_DB_ID, $Info_Master_DB_Password, $Info_Master_DB_PORT, $Info_Master_DB_Name;    
