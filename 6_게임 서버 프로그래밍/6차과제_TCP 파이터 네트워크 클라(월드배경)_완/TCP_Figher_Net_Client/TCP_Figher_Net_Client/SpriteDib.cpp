...
void CSpritedib::GameInit()
{
	// 필요한 스프라이트 로딩해두기.
	// 맵 타일 로딩
	BOOL iCheck = LoadDibSprite(eMAP_TILE, "SpriteData\\Tile_01.bmp", 0, 0);
	if (!iCheck)
		exit(-1);

	// 맵 로딩 (차후 삭제)
	iCheck = LoadDibSprite(eMAP, "SpriteData\\_Map.bmp", 0, 0);
	if (!iCheck)
		exit(-1);
	...
	int iDestHeight, int iDestPitch, BOOL isPlayer, int iDrawLen)
{
	...
	// iDrawX, iDrawY는 스프라이트의 중점을 기준으로 계산한 스프라이트의 x,y위치이다.
	// 비트맵의 가장 좌상단을 구한다. 아직 까지는 픽셀 단위로 계산이다.
	...
	// 위에서 구한 비트맵 좌상단 위치를 기준으로 스프라이트 이미지의, 시작 위치를 구한다. 
	// 클리핑 처리를 하려면, 이미지 찍는 시작 위치를 이동시켜야 한다.
	int iSpriteStartX = 0;
	int iSpriteStartY = 0;
