...
// 2. accountno, 세션키, 수정할 점수 입력받음. 무조건 다 입력했어야 함.
if(isset($_GET['no']) === false || isset($_GET['key']) === false || isset($_GET['score']) === false)
{
...
else
{
    ...
    // 만료 안됐으면 스코어를 갱신한다.
    $Query = "UPDATE `scoretbl` SET `Score` = $NewScore  WHERE `accountno` = $accountno";
    DB_Query($Query);
}
...
// 7. 결과 출력하기
echo '<br>ResultCode : ' . $ReturnCode . '/ AccountNo : ' . $user['accountno'] . '/ New Score : ' . $NewScore . '<br>';
