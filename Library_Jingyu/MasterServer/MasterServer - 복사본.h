...
namespace Library_Jingyu
{
	...
	private:
		...
		// 자료형 : stRoom*, 구현체 : Vector<stRoom*>. 비교 연산자 : 직접 지정(오름차순)
		...
		// 룸을 관리하는 자료구조3
		// Vector 사용
		// !! 방 내의 여유 인원 수가 0이 되었다가 다시 1이 된 방을 관리하는 자료구조 !!
		// !! 4/5인 방을 할당받은 클라이언트가 배틀 방에 들어간 후, 매칭에게 성공패킷을 안보낼 경우, 매칭은 방 입장 실패 패킷을 보낸다 !!
		// !! 실패 패킷을 받으면 마스터 입장에서는 5/5인 방이 4/5가 된 것 !!
		// !! 방 여유 인원수가 생겼으니, pq에 다시 넣는다 !!
		// !! 배틀 입장에서 5/5가 되어서 방 닫힘 패킷이 왔고, umap에서 제거 !!
		// !! 하지만, pq에는 아까 넣었기 때문에 남아있음. 이상하게 됨 !!
		//
		// Key : stRoom*
		vector<stRoom*> m_Room_vector;		

		// m_Room_vector관리용 SRWLOCK
		SRWLOCK m_srwl_Room_vector;
