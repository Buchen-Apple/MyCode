...
{	
	...
	while (1)
	{
		...
		// 3. 헤더의 Len값과 RecvBuff의 데이터 사이즈 비교. (완성 패킷 사이즈 = 헤더 + Len + 엔드코드)
...
// 패킷 타입에 따른 패킷처리 함수
BOOL PacketProc(BYTE PacketType, CProtocolBuff* Packet)
{
	BOOL check;
	try
	{
...
///////////////////////////////////////////////
// [내 캐릭터 생성 패킷] 처리 함수
BOOL PacketProc_CharacterCreate_My(CProtocolBuff* Packet)
{
...
BOOL PacketProc_CharacterCreate_Other(CProtocolBuff* Packet)
{
	// ID, Dir, X, Y, HP 분리
	DWORD	byID;
	BYTE	byDir;
	...
	*Packet >> byHP;
	...
	if (byDir == dfDIRECTION_RIGHT)
		...
		AddPlayer->SetSprite(CSpritedib::ePLAYER_STAND_L01, 5, 5);			
...
// [캐릭터 삭제 패킷] 처리 함수
BOOL PacketProc_CharacterDelete(CProtocolBuff* Packet)
{	
	// ID분리
	DWORD	byID;
	*Packet >> byID;
...
// [다른 캐릭터 이동 시작] 패킷
BOOL PacketProc_MoveStart(CProtocolBuff* Packet)
{
	// ID, Dir, X, Y 분리
	DWORD	byID;
	BYTE	byDir;
...
// [다른 캐릭터 이동 중지] 패킷
BOOL PacketProc_MoveStop(CProtocolBuff* Packet)
{
	// ID, Dir, X, Y 분리
	DWORD	byID;
	BYTE	byDir;
...
// [다른 캐릭터 공격 1번 시작] 패킷
BOOL PacketProc_Atk_01(CProtocolBuff* Packet)
{
	// ID, Dir, X, Y 분리
	DWORD	byID;
	BYTE	byDir;
	...
	*Packet >> byY;
	...
	for (itor = list.begin(); itor != list.end(); itor++)
	{
		if ((*itor)->GetObjectID() == byID)
		{
			...
			// 공격 로직을 처리하라고 명령을 내리는 함수
			AtkPlayer->NonMyAtackCheck(TRUE);
...
// [다른 캐릭터 공격 2번 시작] 패킷
BOOL PacketProc_Atk_02(CProtocolBuff* Packet)
{	
	// ID, Dir, X, Y 분리
	DWORD	byID;
	BYTE	byDir;
...
// [다른 캐릭터 공격 3번 시작] 패킷
BOOL PacketProc_Atk_03(CProtocolBuff* Packet)
{
	// ID, Dir, X, Y 분리
	DWORD	byID;
	BYTE	byDir;
...
// [데미지] 패킷
BOOL PacketProc_Damage(CProtocolBuff* Packet)
{
...
// 좌표 싱크 패킷
BOOL PacketProc_Sync(CProtocolBuff* Packet)
{	
	//printf("sync!\n");	
	...
	*Packet >> Y;
	...
	// 1. 해당 패킷에 날라온 ID의 유저를 찾아낸다.
	CList<BaseObject*>::iterator itor;
	PlayerObject* SyncPlayer;

	for (itor = list.begin(); itor != list.end(); itor++)
	{
		if ((*itor)->GetObjectID() == ID)
		{
			// 2. 패킷에 날라온 Dir을 보고 X,Y와 방향 셋팅
			SyncPlayer = (PlayerObject*)(*itor);

			// x, y 셋팅
			SyncPlayer->MoveCurXY(X, Y);			
