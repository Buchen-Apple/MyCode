...
{ 
    ...
    // 2. Slave에게 연결. $shDB_Index는 인자로 던짐과 동시에 선언
...
function shDB_Data_Update($dbno, $accountno, $TBLName, $Content_Body)
{
    ...
    // 2. DB에 접속. $shDB_Info는 인자로 던지면서 선언된다.
    ...
    // 5. 찾아온 shDB로 커넥트, $shDB_Data 인자로 던지면서 선언된다.
    ...
    // 6. shDB_Data.$TBLName에 해당 accountNo가 있는지 확인 (Slave 에게)
    $Result = DB_Query("SELECT EXISTS (SELECT * FROM `{$dbInfoData['dbname']}`.`$TBLName` WHERE accountno = $accountno limit 1) AS success", $shDB_Data, $accountno);
    if($Result === false)
    {
    ...
    // 8. Update 쿼리문 만들어서 날리기
    $Result = DB_Query("UPDATE `{$dbInfoData['dbname']}`.`$TBLName` SET $SetText WHERE accountno =  $accountno" , $shDB_Data, $accountno);

    // 9. 쿼리 날리는데 실패했으면, 실패 이유에 따라 응답패킷 보낸다.
    if($Result === false)
    {
