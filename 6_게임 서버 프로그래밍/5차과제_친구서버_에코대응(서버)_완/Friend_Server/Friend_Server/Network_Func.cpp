...
// 디버깅용 extern 변수
extern UINT64 g_AcceptCount;							// Accept 한 유저 수
extern UINT64 g_PacketCount;							// 1초에 주고받은 패킷 수. (send 횟수만 체크함. 어차피 recv랑 1:1로 대응되니까)
extern UINT64 g_SendByteCount;							// 1초동안 받은 패킷 바이트 수 
extern UINT64 g_RecvByteCount;							// 1초동안 보낸 패킷 바이트 수 

// Loop 체크용 extern 변수
extern UINT64 g_LoopNowCount;							// 현재 루프 횟수
extern UINT64 g_LoopMinCount;							// 현재 루프횟수를 기준으로 Min값 넣기
extern UINT64 g_LoopAvgCount;							// 현재 루프횟수를 기준으로 Avg값 넣기
extern UINT64 g_LoopMaxCount;							// 현재 루프횟수를 기준으로 Max값 넣기

extern UINT64 g_TotalLoop;								// 현재까지 돈 총 루프 횟수(평균계산용)
extern UINT64 g_AddCount;								// 총 루프 횟수에 더한 횟수(평균계산용)
...
stAccount* ClientSearch_AccountList(UINT64 AccountNo)
{
	...
	iter = map_AccountList.find(AccountNo);
	if (iter == map_AccountList.end())
		return nullptr;
...
{
	stAcceptUser* NowAccount = nullptr;
	map <SOCKET, stAcceptUser*>::iterator iter;

	iter = map_AcceptList.find(sock);
	if (iter == map_AcceptList.end())
		return nullptr;
...
bool Network_Process(SOCKET* listen_sock)
{
	...
	map <SOCKET, stAcceptUser*>::iterator enditor;
	TIMEVAL tval;
	tval.tv_sec = 0;
	tval.tv_usec = 0;
	...
	while (1)
	{
		...
		// 리슨 소켓 처리
		DWORD i = 0;
		if (FD_ISSET(*NowSock[i], &rset) && *listen_sock == *NowSock[i])
		{
...
void Accept(SOCKET* client_sock, SOCKADDR_IN clinetaddr)
{
	...
	// 7. 새로운 유저가 Accept 되면, 증가된 유저 수를 기준으로 루프 횟수 재카운트 위해 변수값 초기화
	g_LoopNowCount = 0;
	g_LoopMinCount = 99999999;
	g_LoopAvgCount = 0;
	g_LoopMaxCount = 0;
	g_TotalLoop = 0;
	g_AddCount = 0;
...
void Disconnect(SOCKET sock)
{
	...
	// 7. 새로운 유저가 접속종료 되면, 감소된 유저 수를 기준으로 루프 횟수 재카운트 위해 변수값 초기화
	g_LoopNowCount = 0;
	g_LoopMinCount = 99999999;
	g_LoopAvgCount = 0;
	g_LoopMaxCount = 0;
...
// 초당 주고받은 패킷 수 리턴 후 기존 TPS는 0으로 초기화
UINT64 TPSCount()
{
	UINT64 temp = g_PacketCount;
	g_PacketCount = 0;
...
bool Network_Req_AccountAdd(SOCKET sock, char* Packet)
{
	...
	*AccountAddPacket >> NickName;
	...
	bool check = SendPacket(&NowAccount->m_SendBuff, &header, &Payload);
	if (check == false)
		return false;
...
bool Network_Req_FriendRemove(SOCKET sock, char* Packet)
{
	...
	*RemovePacekt >> RemoveAccountNo;
	...
	// 7. 만든 패킷을 해당 유저의, SendBuff에 넣기
	bool check = SendPacket(&AcceptUser->m_SendBuff, &header, &Payload);
	if (check == false)
		return false;
...
bool Network_Req_StressTest(SOCKET sock, char* Packet)
{
	...
	*StressTestBuff >> wStringSize;
	...
	memcpy(TestString, StressTestBuff->GetBufferPtr() + 2, wStringSize);

	//2. 페이로드 제작
	// 사이즈만큼 페이로드에 문자열 복사
	CProtocolBuff Payload;
	Payload << wStringSize;
	memcpy(Payload.GetBufferPtr() + 2, TestString, wStringSize);
	Payload.MoveWritePos(wStringSize);

	// 3. 헤더 제작
	CProtocolBuff header(dfHEADER_SIZE);

	BYTE byCode = dfPACKET_CODE;
	WORD MsgType = df_RES_STRESS_ECHO;
	WORD PayloadSize = Payload.GetUseSize();
