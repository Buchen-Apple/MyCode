...
else
{
    ...
    if($timestamp > ($user['StartTime'] + 300))
    {
        ...
        // DB와 연결 해제
        DB_Disconnect($ContentDB);
        exit;
    }

    // 만료 안됐으면 스코어를 갱신한다.
    $Query = "UPDATE `scoretbl` SET `Score` = {$Content_Body['new_score']}  WHERE `accountno` = {$Content_Body['no']}";
    DB_Query($Query, $ContentDB, $Content_Body['no']);
}
...
$Response['new_Score'] = $Content_Body['new_score'];
