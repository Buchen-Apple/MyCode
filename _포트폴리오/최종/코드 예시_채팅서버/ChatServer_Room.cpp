...
#include "CrashDump/CrashDump.h"	// Crash 클래스 include
#include "Log/Log.h"				// Log 저장 클래스 include
#include "Parser/Parser_Class.h"	// Parser 클래스 include
#include "CPUUsage/CPUUsage.h"		// 프로세스의 CPU 사용량 얻는 클래스 include 
#include "PDHClass/PDHCheck.h"		// PDH 정보 얻는 클래스 include 
...
namespace Library_Jingyu
{
	...
	UINT WINAPI CChatServer_Room::HeartBeatThread(LPVOID lParam)
	{
		...
		// 1. 락 경합 감소
		// 2. 데드락 위험 피하기
		// --> Disconnect() 안에서, 셧다운 후에, 바로 IO카운트가 0이 되어, 세션이 릴리즈 될 수 있다.
		// --> 세션이 릴리즈되면 OnClientLeave()를 호출하는데, 이 안에서 Erase하기 위해 다시 락을 건다.
		// --> 즉, 데드락 발생 가능성
		...
		while (1)
		{
			...
			// Size가 있다면, 반복문 돌면서 접속종료 진행
			if (Size > 0)
			{
				// 예를 들어, SessionArray안에 10개의 데이터가 들어있으면 Size도 10
				// Size를 인덱스로 사용할 것이기 때문에 1 감소
				// 따로 변수를 두는 것 보다, '0'과 같은 상수값과 비교하는 것이 훨씬 속도가 빠르기 때문에.
				Size--;
	...
	void CChatServer_Room::RoomClearFunc()
	{
		...
		if (m_Room_Umap.size() > 0)
		{
			...
			{
				// 해당 방에 유저가 있는 경우, 방 안의 모든 유저 접속종료.
	...
	bool CChatServer_Room::ServerStart()
	{
		...
		// Config 파일에서 하트비트 여부가 TRUE라면 하트비트 스레드 생성
	...
	// return : 없음
	void CChatServer_Room::Room_BroadCast(ULONGLONG Array[], int ArraySize, CProtocolBuff_Net* SendBuff)
	{
		// 직렬화 버퍼 레퍼런스 카운트 증가
	...
	void CChatServer_Room::Packet_Login(ULONGLONG SessionID, CProtocolBuff_Net* Packet)
	{
		...
		// 3. 로그인 상태 체크. 
		// 이미 로그인중이면 밖으로 예외 던진다. 로직 실수로 판단
		...
		if (InsertLoginPlayerFunc(AccountNo, SessionID) == false)
		{
			...
			// 2) 유저가 없을 경우 return,
			// 해당 로직을 처리하는 도중 종료되었을 수도 있기 때문에, 없는것도 정상으로 판단.
			if (FindPlayer == m_LoginPlayer_Umap.end())
			{
			...
			// 락을 푸는 순간, 정보가 변조될 가능성이 있기 때문에 필요한 값을 미리 받아둔다.
	...
	void CChatServer_Room::Packet_Room_Enter(ULONGLONG SessionID, CProtocolBuff_Net* Packet)
	{
		...
		// 3. 로그인 상태 체크. 
		// 로그인 중이 아니면 밖으로 예외 던진다. 로직실수로 판단.
	...
	void CChatServer_Room::Packet_Message(ULONGLONG SessionID, CProtocolBuff_Net* Packet)
	{
		...
		// BoradCast		
		Room_BroadCast(SessionArray, ArraySize, SendBuff);
	...
	void CChatServer_Room::Packet_HeartBeat(ULONGLONG SessionID)
	{
		...
		// 3. 로그인 상태 체크. 
		// 로그인 중이 아니면 밖으로 예외 던진다. (클라는 정상적으로 보냈다는 가정)
		if (NowPlayer->m_bLoginCheck == false)
		{
			TCHAR str[100];
			StringCchPrintf(str, 100, _T("Packet_Message(). LoginFlag True. SessionID : %lld, AccountNo : %lld"),
	...
	{
		// 현재는 모든 IP 접속 받는중.
		// 차후 특수한 IP에 대해서 차단시키길 원할 경우, 여기에 추가
		return true;
	}
...
namespace Library_Jingyu
{
	...
	void CChat_LanClient::Packet_RoomErase(ULONGLONG ClinetID, CProtocolBuff_Lan* Payload)
	{
		...
		// 
		// 방이 없는 상황
		// !! 채팅서버가 종료될 경우, 배틀서버는 보유하고 있는 Wait 방을 모두 파괴시킨다. (Play방은 남아있다.) !!
		// !! 다시 채팅서버가 배틀서버에 정상적으로 연결되면, 배틀은 이전의 Play방이 남아있고 채팅은 방이 하나도 없다. !!
		// !! 이 상태에서 게임이 정상적으로 종료되여 Play방 파괴 패킷이 채팅으로 올 경우, 채팅은 다시 켜졌기 때문에 이전의 플레이 방이 없다. !!
		...
		// 4. 유저 접속 종료
		// 현재 접속자 수가 1명 이상이면, 방에 있는 모든 유저 접속종료.
		// 그리고 삭제될 방 플래그 체크
		// 채팅 Net 서버의 OnClientLeave에서 삭제되길 기대한다.
		...
		// 7. 응답 패킷
		// 방에 유저가 남아있었을 경우, 방이 아직 삭제되지 않았을 수도 있지만 곧 삭제될 예정이기 때문에 상관없다.
		CProtocolBuff_Lan* SendBuff = CProtocolBuff_Lan::Alloc();
