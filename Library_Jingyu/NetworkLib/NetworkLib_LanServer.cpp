...
#define dfSENDPOST_MAX_WSABUF			300
	...
	bool CLanServer::Start(const TCHAR* bindIP, USHORT port, int WorkerThreadCount, int ActiveWThreadCount, int AcceptThreadCount, bool Nodelay, int MaxConnect)
	{
		...
		if (retval != 0)
		{
			...
			// 로그 찍기 (로그 레벨 : 에러)
			cLanLibLog->LogSave(false, L"NetServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> WSAStartup() Error : NetError(%d), OSError(%d)",
		...
		if (m_hIOCPHandle == NULL)
		{
			...
			// 로그 찍기 (로그 레벨 : 에러)
			cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> CreateIoCompletionPort() Error : NetError(%d), OSError(%d)",
				(int)m_iMyErrorCode, m_iOSErrorCode);
		...
		for (int i = 0; i < m_iW_ThreadCount; ++i)
		{
			...
			if (m_hWorkerHandle[i] == 0)
			{
				...
				// 로그 찍기 (로그 레벨 : 에러)
				cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> WorkerThread Create Error : NetError(%d), OSError(%d)",
					(int)m_iMyErrorCode, m_iOSErrorCode);
		...
		if (m_soListen_sock == INVALID_SOCKET)
		{
			...
			// 로그 찍기 (로그 레벨 : 에러)
			cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> socket() Error : NetError(%d), OSError(%d)",
				(int)m_iMyErrorCode, m_iOSErrorCode);
		...
		if (retval == SOCKET_ERROR)
		{
			...
			// 로그 찍기 (로그 레벨 : 에러)
			cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> bind() Error : NetError(%d), OSError(%d)",
				(int)m_iMyErrorCode, m_iOSErrorCode);
		...
		if (retval == SOCKET_ERROR)
		{
			...
			// 로그 찍기 (로그 레벨 : 에러)
			cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> listen() Error : NetError(%d), OSError(%d)",
				(int)m_iMyErrorCode, m_iOSErrorCode);
		...
		//	// 로그 찍기 (로그 레벨 : 에러)
		//	cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> setsockopt() SendBuff Size Change Error : NetError(%d), OSError(%d)",
		//		(int)m_iMyErrorCode, m_iOSErrorCode);
		...
		if (Nodelay == true)
		{
			...
			if (retval == SOCKET_ERROR)
			{
				...
				// 로그 찍기 (로그 레벨 : 에러)
				cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> setsockopt() Nodelay apply Error : NetError(%d), OSError(%d)",
					(int)m_iMyErrorCode, m_iOSErrorCode);
		...
		for (int i = 0; i < m_iA_ThreadCount; ++i)
		{
			...
			if (m_hAcceptHandle == NULL)
			{
				...
				// 로그 찍기 (로그 레벨 : 에러)
				cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Start() --> Accept Thread Create Error : NetError(%d), OSError(%d)",
					(int)m_iMyErrorCode, m_iOSErrorCode);
	...
	void CLanServer::Stop()
	{
			...
			retval > WAIT_OBJECT_0 + m_iW_ThreadCount - 1)
		{
			...
			// 로그 찍기 (로그 레벨 : 에러)
			cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Stop() --> Accept Thread EXIT Error : NetError(%d), OSError(%d)",
				(int)m_iMyErrorCode, m_iOSErrorCode);
			...
			retval > WAIT_OBJECT_0 + m_iW_ThreadCount - 1)
		{
			...
			// 로그 찍기 (로그 레벨 : 에러)
			cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"Stop() --> Worker Thread EXIT Error : NetError(%d), OSError(%d)",
				(int)m_iMyErrorCode, m_iOSErrorCode);
		...
		// 7. 서버 종료 로그 찍기		
		cLanLibLog->LogSave(true, L"LanServer", CSystemLog::en_LogLevel::LEVEL_SYSTEM, L"ServerStop...");
	}
	...
	UINT WINAPI	CLanServer::AcceptThread(LPVOID lParam)
	{
		...
		while (1)
		{
			...
			if (client_sock == INVALID_SOCKET)
			{
				...
				// 로그 찍기 (로그 레벨 : 에러)
				cLanLibLog->LogSave(false, L"LanServer", CSystemLog::en_LogLevel::LEVEL_ERROR, L"accpet(). Abonormal_exit : NetError(%d), OSError(%d)",
					(int)g_This->m_iMyErrorCode, g_This->m_iOSErrorCode);
