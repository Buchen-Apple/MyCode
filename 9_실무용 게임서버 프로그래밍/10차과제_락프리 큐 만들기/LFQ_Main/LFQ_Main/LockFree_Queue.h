...
namespace Library_Jingyu
{
	...
	{
		// 내부 노드 셋팅. 최초는 1개 (Interlock으로 감소시키기 때문에 0이되면 아무것도 없는것으로 체크하기 위함.)
		m_NodeCount;
	...
	void CLF_Queue<T>::Enqueue(T data)
	{
		...
		while (true)
		{
				...
				LocalTail.m_l64Count == m_stpTail.m_l64Count)
			{
				...
				{
					if (InterlockedCompareExchangePointer((PVOID*)&m_stpTail.m_pPoint->m_stpNextBlock, NewNode, pLocalNext) == pLocalNext)
					{
						// 성공했으면 이제 Tail 이동작업
	...
	int CLF_Queue<T>::Dequeue(T& OutData)
	{
		...
		alignas(16) st_NODE_POINT LocalHead, TempLocalHead;
		st_LFQ_NODE *pLocalNext, *pDeleteHead, *pLocalTail;

		while (true)
		{
			// head, 이동시도용 tail, Delete노드 스냅샷
			pDeleteHead = TempLocalHead.m_pPoint = LocalHead.m_pPoint = m_stpHead.m_pPoint;
			LocalHead.m_l64Count = m_stpHead.m_l64Count;
			TempLocalHead.m_l64Count = m_stpTail.m_l64Count;

			// head->Next 스냅샷
			pLocalNext = pDeleteHead->m_stpNextBlock;

			// tail 스냅샷
			pLocalTail = m_stpTail.m_pPoint;			

			// 정말 m_stpHead이 LocalHead와 같다면 로직 진행
			if (m_stpHead.m_pPoint == LocalHead.m_pPoint && 
				m_stpHead.m_l64Count == LocalHead.m_l64Count)
			{
				// 헤더와 테일이 같고, pLocalNext가 null인지 체크
				if (pDeleteHead == pLocalTail)
				{
					if (pLocalNext == nullptr)
					{
						m_CDump->Crash();
					...
					{
						// tail 이동 시도			
						InterlockedCompareExchange128((LONG64*)&m_stpTail, LocalHead.m_l64Count + 1, (LONG64)pLocalNext, (LONG64*)&TempLocalHead);
					}
				}
				...
				{
					// out 매개변수에 데이터 셋팅
					OutData = pLocalNext->m_Data;	

					// 헤더 이동 시도
					if (InterlockedCompareExchange128((LONG64*)&m_stpHead, LocalHead.m_l64Count + 1, (LONG64)pLocalNext, (LONG64*)&LocalHead))
					{						
						// 이동 전의 헤더를 메모리풀에 반환
