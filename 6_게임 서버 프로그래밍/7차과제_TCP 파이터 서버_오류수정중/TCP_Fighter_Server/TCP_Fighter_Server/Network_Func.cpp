...
bool Accept(SOCKET* client_sock, SOCKADDR_IN clinetaddr)
{
	...
	NowPlayer->m_wNowX = DEFAULT_X;
	NowPlayer->m_wNowY = DEFAULT_Y;
	...
	Network_Send_CreateMyCharacter(&header, &payload, g_dwPlayerID - 1, dfPACKET_MOVE_DIR_LL, DEFAULT_X, DEFAULT_Y, 100);
...
void DeadReckoning(BYTE NowAction, ULONGLONG ActionTick, int ActionStartX, int ActionStartY, int* ResultX, int* ResultY)
{
	...
	// ----------------------------
	int IntervalFrame = (GetTickCount64() - ActionTick) / 20; // (20이 1프레임)
...
bool RecvProc(stAccount* Account)
{
	...
	while (1)
	{
		...
		BYTE EndCode;		
		int DequeueSize = Account->m_RecvBuff.Dequeue((char*)&EndCode, 1);
		if (DequeueSize == -1)
		{			
	...
	*payload >> Y;
		...
		abs(Account->m_LoginPlayer->m_wNowY - Y) > dfERROR_RANGE)
	{
			...
			abs(resultY - Y) > dfERROR_RANGE)
		{
			...
			// 주변 9섹터에 있는 유저들에게 싱크패킷 보내기
			// 현재 좌표 기준, 섹터 Index 구하기
			int SectorX = resultX / SECTOR_WIDTH;
			int SectorY = resultY / SECTOR_HEIGHT;
			...
			// 9섹터 유저에게 보내기(나 제외)
			SendPacket_Sector(Account->m_LoginPlayer, &header, &payload, &secSector);

			// 나에게 보내기
			if (SendPacket(Account, &header, &payload) == FALSE)
				return FALSE;

			_LOG(dfLOG_LEVEL_DEBUG, L"[DEBUG] MoverStart Sync! [%d, %d] -> [%d, %d]\n",
				X, Y, resultX, resultY);
		}
	...
	*payload >> Y;
		...
		abs(Account->m_LoginPlayer->m_wNowY - Y) > dfERROR_RANGE)
	{
			...
			abs(resultY - Y) > dfERROR_RANGE)
		{
			...
			_LOG(dfLOG_LEVEL_DEBUG, L"[DEBUG] MoverStop Sync! [%d, %d] -> [%d, %d]\n",
				X, Y, resultX, resultY);
