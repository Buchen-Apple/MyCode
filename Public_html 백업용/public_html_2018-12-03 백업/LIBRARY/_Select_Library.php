...
function shDB_Data_Select($dbno, $accountno, $TBLName)
{
    ...
    // 2. DB에 접속. $shDB_Info는 인자로 던짐과 동시에 선언
    ...
    // 5. 찾아온 shDB로 커넥트. $shDB_Data는 인자로 던짐과 동시에 선언
    ...
    // 6. SELECT 쿼리문 만들어서 날리기
    $Result = DB_Query("SELECT * FROM `{$dbInfoData['dbname']}`.`$TBLName` WHERE `accountno` = $accountno" , $shDB_Data, $accountno);

    // 7. 쿼리 날리는데 실패했으면, 실패 이유에 따라 응답패킷 보낸다.
    if($Result === false)
    {
    ...
    // 8. 쿼리에 성공했으면, fetch로 데이터 뽑아온 후 Free.
    ...
    if($RetData === null)
    {
        // 존재하지 않는다면, shDB_Index.allocate에서 해당 accountno의 유저 삭제. 정상 삭제 후 실패 패킷 리턴.
        ...
        // Master DB에 연결. $shDB_Index는 인자로 던짐과 동시에 선언
