...
BOOL RecvProc()
{
	...
	{
		// 리시브 테스트
		int retval = recv(client_sock, RecvBuff.GetBufferPtr() + RecvBuff.GetUseSize()+1, RecvBuff.GetNotBrokenPutSize(), 0);
		if (retval == SOCKET_ERROR)
		{
		...
		//// 에러가 없으면, 받은 데이터는 무조건 리시브 큐에 넣는다. 넣은 후에 해석한다.
