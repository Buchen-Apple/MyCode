...
extern DWORD JoinUser;
extern DWORD SessionDeleteCount;
...
bool SendPost(stSession* NowSession)
{
	...
	if (NowSession->m_SendBuff.GetUseSize() == 0)
	{
		...
		//InterlockedExchange64(&NowSession->m_SendFlag, FALSE);
	...
	// 3. 1칸 이동한 front 위치 알기(실제 front 위치가 이동하지는 않음)	
	...
	if (retval == SOCKET_ERROR)
	{
		...
		if (Error != WSA_IO_PENDING)
		{
			...
			if (Nowval == 0)
			{
				_tprintf(L"SendPost() Disconnect 호출됨\n");
				Disconnect(NowSession);
				return false;
			}
