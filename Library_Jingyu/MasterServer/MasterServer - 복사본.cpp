...
namespace Library_Jingyu
{
	...
	void CMatchServer_Lan::Packet_RoomEnter_Fail(ULONGLONG SessionID, CProtocolBuff_Lan* Payload)
	{
		...
		// 0은 정상, 2는 방 없음 (방 입장 실패 패킷이 왔을 때, 이미 방이 닫혔을 수도 있음)
		if (Ret != 0 && Ret != 2)
			g_MasterDump->Crash();
	}
...
namespace Library_Jingyu
{
	...
	void CBattleServer_Lan::Relay_Battle_Room_Info(ULONGLONG SessionID, UINT64 ClientKey, int MatchServerNo)
	{
		...
		AcquireSRWLockExclusive(&m_srwl_Room_vector);	// ----- vector 룸 Exclusive 락

		// Vector 룸 자료구조에 방 있는지 확인
		if (m_Room_vector.size() > 0)
		{
			// 1. 룸 얻기
			stRoom* NowRoom = m_Room_vector[0];
			...
			ReleaseSRWLockExclusive(&m_srwl_Room_vector);	// ----- vector 룸 Exclusive 언락
		...
		// Vector룸 자료구조에 방이 없을 경우, pq 룸 자료구조 확인
		else
		{
			ReleaseSRWLockExclusive(&m_srwl_Room_vector);	// ----- vector 룸 Exclusive 언락
	...
	CBattleServer_Lan::CBattleServer_Lan()
	{
		...
		InitializeSRWLock(&m_srwl_Room_vector);
